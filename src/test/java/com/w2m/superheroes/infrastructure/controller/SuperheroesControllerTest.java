...
public class SuperheroesControllerTest {
    ...
    public static final String GET_ALL_SUPERHEROES_BY_NAME_URL = "/superheroes?name={name}";
...
"""));
    ...
    void shouldReturnGokuWhenUpdateSuperhero() throws Exception {
        val id = 12;
        mockMvc.perform(post(UPDATE_SUPERHERO_URL, id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"id\": 12, \"name\": \"Goku SSJ2\"}"))
                .andExpect(status().isOk())
                .andExpect(content().json("{\"id\": 12, \"name\": \"Goku SSJ2\"}"));
    }

    @Test
    void shouldReturnOkWhenDeleteSuperhero() throws Exception {
        val id = 12;
        mockMvc.perform(post(DELETE_SUPERHERO_URL, id))
